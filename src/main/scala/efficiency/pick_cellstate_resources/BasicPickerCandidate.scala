...
object BasicPickerCandidate extends CellStateResourcesPicker{
  override def pickResource(cellState: CellState, job: Job, candidatePool: IndexedSeq[Int], remainingCandidates: Int) = {
    ...
    val loop = new Breaks;
    loop.breakable {
      for( i <- (cellState.numMachines - remainingCandidatesVar) to cellState.numMachines-1){
  ...
  override val name: String = "basic-picker-candidate"
}
